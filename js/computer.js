...
import { isColumnFull} from "./board.js"
import { dropPiece,  checkWinner } from "./gameLogic.js";


export function smartComputerTurn() {
    ...
    for (const col of availableColumns) {
        const newNode = createNewNode(gameState.virtualBoard, col, gameState.currentPlayer);/* ゲームの新しい状態を作成 */
    ...
    if (isMaximizingPlayer) {
        ...
        for (let col = 0; col < 7; col++) {
            if (node[0][col] === null) { // isColumnFullの代わりに直接チェック
                const newNode = createNewNode(node, col, 'yellow'); // nodeを使用eval
...
function evaluateLine(compCount, playerCount) {
    ...
    if (compCount === 3) return 77;  // より大きな重み
    if (compCount === 2) return 27;   // より大きな重み
